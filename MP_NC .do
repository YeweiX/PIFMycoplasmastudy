...
table1, vars(careage contn\ rel cat\ ethnicity cate\ regType cat\ marStatus cate\ educ cat\ job cat\ inc cat\ spEduc cat\ spJob cat\ spInc cat\ localRes cat\ numKids cat\ kidage contn\ childSex cat\ childHeight contn\ childWeight contn\preterm cat\ feeding cat\ nutriStatus cat\ chronicCond cat\ healthStat cat\ childPneumoVaccine cat\ childFluTest cat\ childRSVTest cate\ childMycoTest cat\)

*by group
table1, by(Group) vars(rel cat\ cgender cat\ ethnicity cat\ regType cat\ marStatus cat\ educ cat\ job cat\ inc cat\ spEduc cat\ spJob cat\ spInc cat\ localRes cat\ numKids cat\ childSex cat\ childHeight contn\ childWeight contn\preterm cat\ feeding cat\ nutriStatus cat\ chronicCond cat\ healthStat cat\ childPneumoVaccine cat\ childFluTest cat\ childRSVTest cat\ childMycoTest cat\)  
