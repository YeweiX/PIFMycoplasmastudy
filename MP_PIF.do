...
foreach var in heduc incc childSex ssite fever cough {
    di "Interaction analysis for `var'"
    * Unadjusted model with interaction
    xtgee childTestedMycoToday i.Group##i.`var', family(binomial) link(log) eform
    * Adjusted model with interaction
    xtgee childTestedMycoToday i.Group##i.`var' i.ssite i.cough, family(binomial) link(log) eform
    * Store interaction p-value (using test command)
    testparm i.Group#i.`var'
}


***ICC
* Convert Cluster to numeric if needed
capture confirm numeric variable Cluster
if _rc {
    encode Cluster, generate(cluster_id)
}
else {
    gen cluster_id = Cluster
}

* Set clustering
xtset cluster_id

* Fit null model (no clustering)
logit childTestedMycoToday
estimates store null_model

* Fit clustered model
xtmelogit childTestedMycoToday || cluster_id:, variance
estimates store cluster_model

* Likelihood ratio test for cluster effect
lrtest null_model cluster_model
